...
The `onKeyDown` callback only delivers the bare key code, so modifier state must
be queried separately. Use the `getInputHandler()` global inside your handler and
check the `.ctrl`, `.shift`, and `.alt` booleans:

```lua
local function on_key_down(key_code)
    local ih = getInputHandler()
    if key_code == KC_H and ih and ih.shift then  -- Shift + H
        -- do something
    end
end
registerHandler("onKeyDown", on_key_down)
